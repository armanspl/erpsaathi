<template>
    <div class="space-y-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">Categories</h1>
                <Breadcrumb :items="['Dashboard', 'Library', 'Categories']" class="mt-1" />
            </div>
...
async function load() {
    loading.value = true;
    const { data } = await client.get('/library/categories');
...
async function save() {
    ...
    try {
        if (editing.value) {
            await client.put(`/library/categories/${editing.value.id}`, form);
            pushToast('Category updated.', 'success');
        } else {
            await client.post('/library/categories', form);
...
async function remove(category) {
    ...
    await client.delete(`/library/categories/${category.id}`);
